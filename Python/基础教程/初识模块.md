...
* 解释器在解释执行时会自动为每个py文件设置独立的命名空间,也就是说可以包含相同声明而互不冲突
...
* 由于此文件包含业务逻辑所以可以暂定为脚本

![image-20190817143803281](初识模块.assets/image-20190817143803281.png)

* 解释器在解释执行时会自动为每个py文件设置独立的命名空间
* 导入的本质是变量赋值的过程,如上解释器首先解释执行course_005.py文件并为其创建独立的命名空间course_005,当解释执行至import course_004时自动搜索解释执行course_004.py文件并为其创建独立的命名空间course_004,并隐式的在命名空间course_005中创建一个变量名为course_004指向的是命名空间course_004,至此在命名空间course_005中可通过变量course_004访问命名空间course_004中的一切声明
...
* 解释器会默认按照如上路径顺序查找,首行空字符串表示当前目录,这也就是上面例子可以正常运行的关键原因
* 由于解释器查找模块是按照sys.path顺序查找,也就是说其实我们可以通过修改此列表来导入任意目录的模块
...
* Python中\_\_name\_\_叫做魔术变量,值为当前模块名,作为脚本执行时由于\_\_name_\_的值为\_\_main\_\_所以语句块儿的代码会被执行,而作为模块导入时由于\_\_name\_\_的值为course_004所以语句块儿的代码不会被执行
* 由于此特性可以统一模块和脚本的概念,于是也就约定俗成无论脚本还是模块都应该按照这个规范编写.

## 内置模块

> Python包含众多内置模块,使用方法和自定义模块完全一样,点击进入[Python2内置模块列表](https://docs.python.org/2/library/index.html),[Python3内置模块列表](https://docs.python.org/zh-cn/3.7/library/index.html)
